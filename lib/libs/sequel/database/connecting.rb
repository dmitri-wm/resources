...
module Sequel
  class Database
    ...
    ADAPTERS = %w[ado amalgalite ibmdb jdbc mock mysql mysql2 odbc oracle postgres sqlanywhere sqlite tinytds
                  trilogy].map(&:to_sym)
    ...
    def self.connect(conn_string, opts = OPTS)
      ...
      when String
        if conn_string.start_with?("jdbc:")
          ...
          uri.query.split("&").map do |s|
            s.split("=")
          end.each { |k, v| uri_options[k.to_sym] = v if k && !k.empty? } unless uri.query.to_s.strip.empty?
      ...
      begin
        db = c.new(opts)
        return yield(db) if defined?(yield)
      ensure
        if defined?(yield)
          db.disconnect if db
    ...
    def add_servers(servers)
      unless sharded?
        raise Error,
              "cannot call Database#add_servers on a Database instance that does not use a sharded connection pool"
      end
    ...
    def remove_servers(*servers)
      unless sharded?
        raise Error,
              "cannot call Database#remove_servers on a Database instance that does not use a sharded connection pool"
      end
