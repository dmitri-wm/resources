...
module Sequel
  ...
  module SequelMethods
    ...
    def extension(*extensions)
      extensions.each { |e| require("extensions/#{e}") }
    end
    ...
    # not be provided.
    def synchronize_with(mutex, &block)
      if mutex
      ...
      unless v = Sequel.synchronize { SPLIT_SYMBOL_CACHE[sym] }
        v = if split_symbols?
              ...
              when /\A((?:(?!__).)+)__((?:(?!___).)+)___(.+)\z/
                [::Regexp.last_match(1).freeze, ::Regexp.last_match(2).freeze, ::Regexp.last_match(3).freeze].freeze
              when /\A((?:(?!___).)+)___(.+)\z/
                [nil, ::Regexp.last_match(1).freeze, ::Regexp.last_match(2).freeze].freeze
              when /\A((?:(?!__).)+)__(.+)\z/
                [::Regexp.last_match(1).freeze, ::Regexp.last_match(2).freeze, nil].freeze
              else
