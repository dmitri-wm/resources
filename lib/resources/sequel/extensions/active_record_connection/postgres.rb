    ...
    module Postgres
      ...
      def transaction(opts = OPTS)
        %i[deferrable read_only synchronous].each do |key|
          raise Error, "#{key.inspect} transaction option is currently not supported" if opts.key?(key)
        end

        super
      rescue StandardError => e
        if e.instance_of?(::ActiveRecord::PreparedStatementCacheExpired) && !in_transaction?
          activerecord_connection.clear_cache!
        end
        raise
      end
