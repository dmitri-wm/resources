...
module Sequel
  ...
  module SequelMethods
    ...
    def convert_timestamp(v, input_timezone)
      if v.is_a?(Date) && !v.is_a?(DateTime)
        ...
        if datetime_class == DateTime
          DateTime.civil(v.year, v.month, v.day, 0, 0, 0,
                         application_timezone == :local ? Rational(Time.local(v.year, v.month, v.day).utc_offset, 86_400) : 0)
        else
    ...
    def convert_input_datetime_other(_v, input_timezone)
    ...
    # by default (i.e. one other than +nil+, <tt>:local</tt>, or <tt>:utc</tt>).  Raises an +InvalidValue+ by default.
    # Can be overridden in extensions.
    def convert_input_time_other(_v, input_timezone)
      raise InvalidValue, "Invalid input_timezone: #{input_timezone.inspect}"
    end
    ...
    def convert_input_timestamp(v, input_timezone)
      ...
      when Array
        ...
        if datetime_class == DateTime
          s += Rational(ns, 1_000_000_000) if ns
          if off
          ...
          s += Rational(ns, 1_000_000_000) if ns
          Time.new(y, mo, d, h, mi, s, (off * 86_400).to_i)
        else
          case input_timezone
          when nil, :local
      ...
      when Hash
        ary = %i[year month day hour minute second nanos].map { |x| (v[x] || v[x.to_s]).to_i }
    ...
    # by default (i.e. one other than +nil+, <tt>:local</tt>, or <tt>:utc</tt>).  Raises an +InvalidValue+ by default.
    # Can be overridden in extensions.
    def convert_output_datetime_other(_v, output_timezone)
    ...
    # by default (i.e. one other than +nil+, <tt>:local</tt>, or <tt>:utc</tt>).  Raises an +InvalidValue+ by default.
    # Can be overridden in extensions.
    def convert_output_time_other(_v, output_timezone)
      raise InvalidValue, "Invalid output_timezone: #{output_timezone.inspect}"
    end
    ...
    def time_offset_to_datetime_offset(offset_secs)
      ...
      Sequel.synchronize { @local_offsets[offset_secs] = Rational(offset_secs, 86_400) }
    end
  end
end
