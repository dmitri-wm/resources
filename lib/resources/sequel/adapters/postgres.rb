...
begin
  ...
  raise LoadError if !defined?(PG::Connection) && !defined?(PGconn::CONNECTION_OK)
  ...
  module Postgres
    ...
    class Adapter < PGconn
      ...
      if USES_PG
        ...
        # :nocov:
        alias async_exec_params async_exec unless public_method_defined?(:async_exec_params)
        ...
        def escape_bytea(str)
          str.gsub(/[\000-\037\047\134\177-\377]/n) do |b|
            "\\#{format("%o", b.each_byte do |x|
                                break x
                              end).rjust(3, "0")}"
          end
        end
    ...
    class Database < Sequel::Database
      ...
      def bound_variable_arg(arg, _conn)
        case arg
      ...
      def connect(server)
        ...
        if USES_PG
          ...
          }.delete_if { |_key, value| blank_object?(value) }
          # :nocov:
      ...
      if USES_PG
        ...
        def copy_table(table, opts = OPTS)
          synchronize(opts[:server]) do |conn|
            ...
            begin
              ...
              raise PG::NotAllCopyDataRetrieved, "Not all COPY data retrieved" if !res || res.result_status != 1
              ...
              if buf && !e
                raise DatabaseDisconnectError,
                      "disconnecting as a partial COPY may leave the connection in an unusable state"
              end
            end
          end
        end
        ...
        def copy_into(table, opts = OPTS)
          ...
          raise Error, "Cannot provide both a :data option and a block to copy_into" if defined?(yield) && data
      ...
      def _execute_prepared_statement(conn, ps_name, args, _opts)
      ...
      def database_exception_sqlstate(exception, _opts)
        # :nocov:
        return unless exception.respond_to?(:result) && (result = exception.result)
      ...
      # of rows changed.
      def execute_prepared_statement(conn, name, opts = OPTS)
        ...
        unless conn.prepared_statements[ps_name] == sql
          ...
          conn.check_disconnect_errors do
            log_connection_yield("PREPARE #{ps_name} AS #{sql}", conn) do
        ...
        q = conn.check_disconnect_errors do
          log_connection_yield(log_sql, conn, args) do
            _execute_prepared_statement(conn, ps_name, args, opts)
          end
        end
        begin
          defined?(yield) ? yield(q) : q.cmd_tuples
    ...
    class Dataset < Sequel::Dataset
      ...
      def paged_each(opts = OPTS, &block)
        return enum_for(:paged_each, opts) unless defined?(yield)
...
if Sequel::Postgres::USES_PG && !ENV["NO_SEQUEL_PG"]
  begin
    require "sequel_pg"
    if defined?(Gem) &&
       (sequel_pg_spec = begin
         Gem.loaded_specs["sequel_pg"]
       rescue StandardError
         nil
       end) &&
